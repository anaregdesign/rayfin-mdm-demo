...
`npx -y @microsoft/rayfin-cli docs ...` from the project root). The bundled
skill, the `rayfin` entry in `.mcp.json`, and `rayfin/.lockfile.json` are
CLI-managed and re-synced — never hand-edit them. When platform questions
...
rayfin/                         # PLATFORM — owned by the `rayfin` skill + MCP
  ...
  .env                          # source for generated .env.local (`rayfin env`)
  .lockfile.json                # CLI scaffolding-sync hashes — never hand-edit
```
...
- Bootstrap new projects with `npm create @microsoft/rayfin@latest`; it
  ...
  Tailwind + Vite wiring, `rayfin/` schema scaffold, `AGENTS.md`, and
  `.mcp.json`. Do not retrofit a plain Vite template or enable
  `experimentalDecorators`. `.env.local` is generated by `rayfin env` from
  `rayfin/.env` on every dev/build — never hand-edit it.
