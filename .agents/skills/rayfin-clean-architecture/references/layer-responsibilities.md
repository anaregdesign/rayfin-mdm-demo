...
- `.env.local` is generated by `rayfin env` from `rayfin/.env`
  (`RAYFIN_PUBLIC_<NAME>` → `VITE_RAYFIN_<NAME>`) on every dev/build. Treat it
  as build output: never hand-edit it, and change values in `rayfin/.env`
  (platform-owned) instead.
